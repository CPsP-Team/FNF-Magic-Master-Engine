...
class MobileUtil
{
	...
	public static inline function getAssetDirectory():String
		return #if android haxe.io.Path.addTrailingSlash("/sdcard/Android/data/ccom.yirius.magic/files") #elseif ios lime.system.System.documentsDirectory #else Sys.getCwd() #end;
	...
	public static function initDirectory():String {
		...
		if (!FileSystem.exists(getStorageTypePath()))
			File.saveContent(getStorageTypePath(), Settings.get('Storage Type'));
		...
		switch(curStorageType) {
			case 'EXTERNAL':
				daPath = "/sdcard/.Magic-Master-Engine";
			case 'EXTERNAL_MEDIA':
				daPath = "/sdcard/Android/media/com.yirius.magic";
			case 'EXTERNAL_DATA':
				daPath = "/sdcard/Android/data/com.yirius.magic/files";
			default:
				if (daPath == null || daPath == '') daPath = "/sdcard/Android/data/com.yirius.magic/files";
		}
