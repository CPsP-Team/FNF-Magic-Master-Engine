...
class Mods {
    ...
    public static function save():Void {
        Mods.saved = []; for(mod in Mods.list){Mods.saved.push({name: mod.name, enabled: mod.enabled});}
        FlxG.save.data.mods = saved; FlxG.save.flush();
        ...
        for(mod in list){
            ...
            for(i in 0...script_list.length){
                ...
                if(cur_script == "ModData"){Mods.mod_scripts.set(mod.name, new_script); continue;}
    ...
    public static function get(_name:String):Mod {
        for(mod in list){ if(mod.name != _name){continue;} return mod; }
        return null;
    }
