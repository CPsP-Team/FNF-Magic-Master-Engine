...
class Paths {
	...
	inline static public function preset(key:String, ?mod):String {return getPath('${key}.json', TEXT, 'notes', mod);}
	...
	inline static public function events(jsonInput:String, ?mod:String):String {
		var path = getPath('songs/${jsonInput.split('-')[0]}/events.json', TEXT, 'data', mod);
		if(!Paths.exists(path)){path = getPath('songs/Test/events.json', TEXT, 'data', mod);}
		return path;
	}
	...
	inline static public function _character(char:String, key:String, type:AssetType = TEXT, ?mod:String):String {
		return getPath('${char}/${key}', type, 'data', mod);
	}
