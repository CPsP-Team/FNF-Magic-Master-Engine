...
class Song {
	...
	public static function parse(song_data:Song_File, ?event_data:Event_File):Void {
		...
		if(song_data.speed <= 0){song_data.speed = 3.0;}
		...
		if(song_data.strums.length <= 0){song_data.strums.push({playable: true, style: "Default", keys: 4, characters:[], sections:[]});}
				...
				for(section in strum.sections){
					...
					if(section.notes == null){
						...
						for(note in section.notes){
							Note.set_note(note, Note.convNoteData(Note.getNoteData(note)));
						}
					}
				}
			}
		...
		for(section in song_data.sections){
			if(section.lengthInSteps <= 0){section.lengthInSteps = 16;}
		}
		...
		if(event_data != null){
			for(event in event_data.events){
				...
				for(check_event in song_data.events){
					if(Note.compare(cur_data, Note.getEventData(check_event), false)){
						has_note = true;
						break;
					}
				}
