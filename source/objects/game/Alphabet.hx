...
class Alphabet extends FlxUIGroup {
    ...
    public function popScore(_score:Int, _style:String, _scale:Float = 0.5):Void {
        ...
        for(_digit in '$_score'.split("")){
            ...
            FlxTween.tween(_number, {y: _number.y - 35, alpha: 0, angle: FlxG.random.float(-25, 25)}, 0.5 + (_current * 0.2), {ease: FlxEase.quadOut, onComplete: (twn)->{remove(_number, true).destroy();}});
